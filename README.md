...
└────────────────────────────┬─────────────────────────────────────┘
                             │
┌─────────────┬──────────────▼──────────────┬────────────────────┬──────────────┐
│  知识检索层  │       工具调用层              │    可观测性         │    测试评估层   │
│             │                             │                    │              │
│  RAGEngine  │  ToolManager               │  OpenTelemetry     │  GoldenTest  │
│  混合检索   │  └─ MCPManager              │  AgentEvaluator    │  Regression   │
│  HyDE/重排  │       └─ MCP Server         │  Tracer            │  RAGEvaluator │
│  查询重写   │          购物平台后端         │                    │  EvalStore    │
│  4级路由    │          (商品/订单/用户)    │                    │              │
└─────────────┴──────────────┬──────────────┴────────────────────┴──────────────┘
                             │
┌────────────────────────────▼─────────────────────────────────────┐
...
llm:
  ...
  # 或使用 OpenAI/其他兼容 OpenAI API 的服务
  # provider: "openai"
...
启动 Web 界面后，在浏览器中访问 `http://localhost:8080` 即可使用图形化聊天界面。
...
| `/new` | 创建新会话 |
| `/switch <编号>` | 切换到历史会话 |
| `/delete <编号...>` | 删除指定会话（如 /delete 1 3 5） |
| `/delete_all` | 删除所有会话 |
| `/clear` | 清空短期记忆 |
| `/reset` | 重置会话 |
...
│
├── auth/                            # 🔐 用户认证（登录/Token）
│   └── auth_manager.py              #    认证管理器（登录/Token生成/验证）
├── session/                         # 📋 会话管理（SQLite，含 task_states 独立表）
│   └── session_manager.py           #    会话管理器（创建/查询/删除/恢复）
├── mcp_manager_module/              # 🔧 MCP 协议管理 + 购物平台后端服务
│   ├── mcp_manager.py               #    MCP 管理器（SSE连接/工具发现/调用）
│   └── test_server_sse.py           #    测试MCP服务器（模拟购物平台后端）
├── context/                         # 📐 上下文窗口工程（分区预算管理）
│   └── context_manager.py           #    上下文窗口管理器（预算分配/裁剪/投影）
├── input_gate/                      # 🛡️ 输入安全验证
│   └── input_validator.py           #    输入验证器（注入检测/敏感词过滤）
├── security/                        # 🔒 输出审查（output_guard）+ 频率限制（rate_limiter）
│   ├── output_guard.py              #    输出审查器（敏感信息脱敏/禁止内容拦截）
│   └── rate_limiter.py              #    频率限制器（全局/单工具/单会话限制）
...
│   └── tracer.py                    #    OpenTelemetry 追踪器
├── tool_manager/                    # 🧰 统一工具管理（本地工具 + MCP 工具，多层过滤）
│   ├── tool_manager.py              #    工具管理器（工具注册/过滤/调用）
...
│   ├── app.py                       #    FastAPI 应用（登录/聊天/会话管理 API）
│   └── static/                      #    前端静态资源
...
| 配置项 | 说明 | 示例 |
|--------|------|------|
| `llm.provider` | LLM 提供商类型 | `ollama` / `openai` |
| `llm.base_url` | LLM 服务地址 | `http://localhost:11434/v1` |
| `llm.model` | 模型名称 | `qwen2.5:7b` |
| `llm.api_key` | API 密钥 | `EMPTY` (Ollama) / `sk-xxx` (OpenAI) |
| `embedding.provider` | 嵌入模型提供商 | `local` / `ollama` / `openai` |
| `embedding.model` | 嵌入模型路径或名称 | `../data/model/bge-small-zh-v1.5` |
| `embedding.dimension` | 嵌入向量维度 | `512` (bge-small-zh-v1.5) |
...
| `orchestrator.sub_agent_defaults.timeout` | 子 Agent 执行超时（秒） | `60` |
| `security.human_confirmation` | 敏感操作确认级别 | 下单 confirm / 删除 double |
| `react.max_iterations` | ReAct 最大迭代轮次 | `10` |
| `planner.max_replan_attempts` | 评估失败最大重规划次数 | `2` |
| `rag.hybrid_search.enabled` | 是否启用混合检索（向量+BM25） | `true` |
...
MCP 服务配置：`config/mcp_servers.yaml`

配置 MCP 服务端点和工具分类，支持动态添加多个 MCP 服务。
...
| LLM 连接失败 | 检查 `settings.yaml` 中的 `provider`、`base_url` 和 `model`，确认 LLM 服务在线 |
| 登录失败 | 先运行 `python -m init.init_db` 初始化用户数据 |
| Agent 部分功能不可用 | 调用 `agent.get_health_status()` 查看各组件状态，确认降级组件 |
| 并行阶段执行超时 | 调整 `config/settings.yaml` 中子 Agent 超时配置（默认 60s/阶段） |
| 评估测试全部失败 | 确认 Agent 已正常启动，或加 `--category intent` 单独跑意图识别测试 |
| RAG 评估分数低 | 运行 `python -m init.import_product_kb --force` 确认知识库已导入 |
| Web界面无法访问 | 确认 FastAPI 服务已启动：`uvicorn web.app:app --reload --port 8080` |
| 嵌入模型加载失败 | 检查 `settings.yaml` 中 `embedding.model` 路径是否正确，或使用 Ollama/OpenAI 嵌入服务 |
